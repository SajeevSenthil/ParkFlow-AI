...
Translate the forecast into **rupees of commuter productivity lost**: predicted violations → estimated road-capacity loss (reuse 8.2) → commuter-hours of delay → ₹, using the **single economic source** — the Bengaluru congestion study, Vijayalakshmi S & Krishna Raj (2023), *Estimation of Productivity Loss Due to Traffic Congestion: Evidence from Bengaluru City*, ISEC Working Paper 554. Value of time ≈ **₹164/commuter-hour (2018-19)** is derived from that paper (Table 2: ₹11,45,568 ÷ 6,998 hours lost ≈ ₹163.7/hr; cross-checked vs ₹34,952/mo ÷ 208 h ≈ ₹168/hr). The paper is a **per-commuter** model, so occupancy = 1.0; `vehicles_blocked_per_violation` and `max_delay_hours_per_vehicle` are the project's own **tunable modelling assumptions**, not figures from the paper. Real-world scale anchor (same paper): city-wide ≈ 7.07 lakh hours / ≈ ₹11.7 billion lost in 2018. `economics.economic_impact()` → `economic_impact.csv` + `economic_summary` in `metrics.json`. No external data.
